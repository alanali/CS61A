...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    valid = [i for i in paragraphs if (select(i))]
    if k >= len(valid):
        return ''
    return valid[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    def contain(s):
        ...
        within = [i in words for i in topic]
        return True in within
    return contain
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    # nested list of each valid word and difference between typed word
    diffs = [[valid, diff_function(typed_word, valid, limit)] for valid in valid_words]
    # list of clostest valid word and its difference
    low_diff = min(diffs, key=lambda difference: difference[1])
    # return word if difference less than limit
    return low_diff[0] if low_diff[1] <= limit else typed_word
    # END PROBLEM 5
...
def feline_flips(start, goal, limit):
    ...
    # if the first letters of both words are not the same
    if start[0] != goal[0]:
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit == 0:
        return 1
    # if either string in empty
    if not start or not goal:
        return max(len(start), len(goal))
    # if first letters are the same, go to next letter
    if start[0] == goal[0]:
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    scoreboard = [[] for i in player_indices]
    for word in word_indices:
        ...
        for player in player_indices:
            ...
            if player_time < fastest:
                fastest = player_time
                player_num = player
        scoreboard[player_num] += [word_at(match, word)]
    return scoreboard
    # END PROBLEM 10

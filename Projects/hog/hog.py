...
def picky_piggy(score):
    ...
    if score == 0:
        return 7
    deci = make_test_dice(1, 4, 2, 8, 5, 7)     # repeating decimal digits of 1/7
    index = score % 6
    if index == 0:
        return 7
    while index > 0:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        last = score1 if who else score0
        increase = last - last_score
        if increase > running_high:
            print(str(increase) + " point(s)! That's a record gain for Player " + str(who) +'!')
        return announce_highest(who, last, max(increase, running_high))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_dice, highest, max_roll = 1, 0, 0
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hogpile = score + picky_piggy(opponent_score)
    if hog_pile(hogpile, opponent_score):       # if hogpile takes effect
        return 0
    else:

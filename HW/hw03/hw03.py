...
def num_eights(pos):
    ...
    if pos == 0:
        return 0
    if abs(pos) % 10 == 8:
        return 1 + num_eights(abs(pos)//10)
    return num_eights(abs(pos)//10)
...
def pingpong(n):
    ...
    def stairs(a, b, c):
        if n == c:
            return a
        if (num_eights(c)) or (c%8 == 0):
            print("DEBUG", "SWITCH", a, b, c)
            return stairs(a-b, b*(-1), c+1)
        else:
            print("DEBUG", a, b, c)
            return stairs(a+b, b, c+1)
    return stairs(1, 1, 1)
...
def count_coins(change):
    ...
    def ways(mon, coin):
        if mon == 0 or mon == 1:
            return 1
        if mon < coin:
            return ways(mon, descending_coin(coin))
        if coin == 1:
            return 1
        else:
            return ways(mon, descending_coin(coin)) + ways(mon-coin, coin)

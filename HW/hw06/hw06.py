    ...
    def vend(self):
        if self.stock:
            if self.funds > self.price:
                ...
                self.funds = 0
                self.stock -= 1
                return f'Here is your {self.name} and ${current - self.price} change.'
...
class Coin:
    ...
    def worth(self):
        print("DEBUG:", self.cents, Mint.present_year, self.year)
...
def two_list(vals, amounts):
    ...
    if vals:
        if amounts[0] > 1:
            ...
            ll = Link(vals[0], two_list(vals, amounts))
        else:
            ll = Link(vals[0], two_list(vals[1:], amounts[1:]))
    return ll
